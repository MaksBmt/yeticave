...
SELECT l.title, l.start_price, l.img, name_category FROM lots l JOIN categories c ON l.category_id=c.id;

--Показываем лот по его ID и получаем название категории, к которой принадлежит лот
SELECT l.id, l.title, name_category 
FROM lots l 
JOIN categories c 
